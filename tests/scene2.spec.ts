...
test('Memesan pada jam 09:00 - 11:00', async ({ page }) => {
    ...
    await page.locator('#start_time').selectOption('09:00'); // choose start time
    await page.locator('#end_time').selectOption('11:00'); // choose end time
    await expect(page.locator('#price')).toHaveText('Rp. 1.000.000'); // make sure the price displayed is correct
    ...
    await expect(page.locator('#time_ordered')).toContainText('09:00 - 11:00'); // make sure the time is correct
    await expect(page.locator('#price_ordered')).toContainText('Rp. 1.000.000'); // make sure the price is correct
});
